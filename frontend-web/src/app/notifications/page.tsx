...
const ICON_COLOR: Record<string, string> = {
  recycling: "bg-primary-container/30 text-primary",
  redeem: "bg-secondary-container/30 text-secondary",
  chat: "bg-tertiary-container/30 text-tertiary",
  notifications: "bg-surface-container text-on-surface-variant",
};
...
export default function NotificationsPage() {
  ...
  useEffect(() => {
    getNotifications().then((historical) => {
  ...
  // Inject newly arrived real-time notifs into local state
  useEffect(() => {
    const seen = seenIdsRef.current;
    const fresh = realtimeNotifs.filter((n) => !seen.has(n.id));
  ...
  return (
    <AppShell>
      <div className="max-w-[800px] mx-auto px-container-padding-mobile md:px-container-padding-desktop py-4 md:py-stack-lg">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-xl md:text-headline-lg font-headline-lg text-primary">{t("notifications.title")}</h1>
